...
def update_graph_tab(Year, Country, Agency, City, Property_Name, Class, SQM, Business_Sector, Type_of_Deal,
    ...
    for agency in ['Colliers', 'CW', 'CBRE', 'JLL','KF', 'SAR']:
        a = list(((pv[("SQM", agency)] / 1000).round()).apply(np.int64))
        print(agency, a)

    for year, value in zip(pv.index, list(((pv[("SQM", 'Colliers')] / 1000).round()).apply(np.int64))):
        #print('value Colliers =', value)
        #print('PV SQM SUM Colliers', list(((pv[("SQM", 'Colliers')] / 1000).round()).apply(np.int64)))
    ...
    for year, value, value_ex in zip(pv.index, list(((pv[("SQM", 'CW')] / 1000).round()).apply(np.int64)), (((pv[("SQM", 'Colliers')] / 1000).round()).apply(np.int64)) ):
        #print('value CW =', value)
        #print('PV SQM SUM CW ', list(((pv[("SQM", 'CW')] / 1000).round()).apply(np.int64)))
        #print('y coordinte=', (((pv[("SQM", 'Colliers')] / 1000).round()).apply(np.int64))+(value*1000)/2)
        annotations.append(dict(
            x=year,
